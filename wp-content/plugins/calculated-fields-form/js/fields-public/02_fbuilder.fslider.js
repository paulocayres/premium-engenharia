						...
						function typeValuesComponents()
						{
							function component(c)
							{
								...
								$(document).on('keyup change', '#'+me.name+'_component_'+c, function(){
									...
									timeoutId = setTimeout(function(){$('#'+me.name+'_slider').slider(
									((v != undefined) ? 'value' : 'values'), v || [Math.min(v1,v2), Math.max(v1,v2)]);
									me._setFieldValue();}, 500);
								});
								return '<div class="slider-type-'+c+'-component"><input id="'+me.name+'_component_'+c+'" class="large" type="number" value="'+predefined+'" min="'+min+'" max="'+max+'" step="'+step+'" /></div>';
							};
